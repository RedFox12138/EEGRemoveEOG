%% VME_EFD 真实数据集测试脚本
% 使用VME_EFD（Variational Mode Extraction with Empirical Fourier Decomposition）方法
% 对真实数据集进行EOG伪影去除
% 输出统一格式的.mat文件
...
% 结果保存路径
RESULTS_DIR = 'results';
PREDICTION_SAVE_PATH = fullfile(RESULTS_DIR, 'VME_EFD_real_data_predictions.mat');

% 总结果保存路径
FINAL_RESULTS_DIR = fullfile('..', '..', '..', 'results');
FINAL_PREDICTION_PATH = fullfile(FINAL_RESULTS_DIR, 'VME_EFD_real_data_predictions.mat');
...
for i = 1:n_samples
    ...
    try
        % 调用VME_EFD去噪函数
        cleaned_signal = vme_efd_denoise(signal, SAMPLING_RATE);
