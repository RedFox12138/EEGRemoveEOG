...
MODEL_SAVE_PATH = os.path.join(CHECKPOINT_DIR, f'EEGIFNet_{DATASET_NAME}_best.pth')
FINAL_MODEL_PATH = os.path.join(CHECKPOINT_DIR, f'EEGIFNet_{DATASET_NAME}_final.pth')

# ========== 结果保存路径 ==========
RESULTS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'results')
os.makedirs(RESULTS_DIR, exist_ok=True)

PREDICTION_SAVE_PATH = os.path.join(RESULTS_DIR, f'EEGIFNet_{DATASET_NAME}_predictions.mat')


def print_data_config():
    """打印当前配置信息"""
    print("=" * 80)
    print(f"EEGIFNet 数据配置信息")
    print("=" * 80)
    print(f"\n[数据集配置]")
