...
class Decoder(Module):
    ...
    def _align_and_cat(self, x, skip):
        """使用插值对齐x和skip的尺寸，然后拼接"""
        # 确保都是3D tensor
        if len(x.shape) == 2:
            x = x.unsqueeze(1)
        if len(skip.shape) == 2:
    ...
    def forward(self, x, skip_states, eegart_skip_states, cbam_skip_states, origsig):
        ...
        x = self._align_and_cat(x, eegart_skip_states[-1])
        x = self.post_latent2(x)
        #print(f'post-latent2 shape: {x.shape}')
        
        x = self._align_and_cat(x, cbam_skip_states[-1])
        x = self.post_latent2_CBAM(x)

        #print(f'post-latent2 shape: {x.shape}')
        x = self._align_and_cat(x, eegart_skip_states[-2])
        x = self.post_latent1(x)
        
        x = self._align_and_cat(x, cbam_skip_states[-2])
        x = self.post_latent1_CBAM(x)
        
        #print(f'post-latent1 shape: {x.shape}')
        x = self._align_and_cat(x, skip_states[-1])
        x = self.post_latent0(x)
        
        x = self._align_and_cat(x, cbam_skip_states[-3])
        ...
        # 使用插值对齐尺寸
        l_skip, m_skip, h_skip = skip_arr[0], skip_arr[1], skip_arr[2]
        if arr[0].shape[2] != l_skip.shape[2]:
            l_skip = torch.nn.functional.interpolate(l_skip, size=arr[0].shape[2], mode='linear', align_corners=False)
            m_skip = torch.nn.functional.interpolate(m_skip, size=arr[1].shape[2], mode='linear', align_corners=False)
            h_skip = torch.nn.functional.interpolate(h_skip, size=arr[2].shape[2], mode='linear', align_corners=False)
        
        l = torch.concat([arr[0], l_skip], dim=1)
        m = torch.concat([arr[1], m_skip], dim=1)
        h = torch.concat([arr[2], h_skip], dim=1)
        ...
        # 使用插值对齐尺寸
        l_skip, m_skip, h_skip = skip_arr[0], skip_arr[1], skip_arr[2]
        if l.shape[2] != l_skip.shape[2]:
            l_skip = torch.nn.functional.interpolate(l_skip, size=l.shape[2], mode='linear', align_corners=False)
            m_skip = torch.nn.functional.interpolate(m_skip, size=m.shape[2], mode='linear', align_corners=False)
            h_skip = torch.nn.functional.interpolate(h_skip, size=h.shape[2], mode='linear', align_corners=False)
        
        l = torch.concat([l, l_skip], dim=1)
        m = torch.concat([m, m_skip], dim=1)
        h = torch.concat([h, h_skip], dim=1)
        ...
        # 确保输出和输入长度一致
        if x.shape[2] != origsig.shape[2]:
            x = torch.nn.functional.interpolate(x, size=origsig.shape[2], mode='linear', align_corners=False)
        ...
        if n_windows > 0:
            # 只处理能被16整除的部分
            x_windowed = x[:, :, :n_windows*16].view(B, 1, n_windows, 16)
            x_windowed = x_windowed * weights[..., None]
            x_windowed = x_windowed.view(B, 1, n_windows*16)
            
            # 如果有余数，直接拼接
            if n_windows*16 < N:
                x_remainder = x[:, :, n_windows*16:]
                x = torch.cat([x_windowed, x_remainder], dim=2)
            else:
                x = x_windowed
