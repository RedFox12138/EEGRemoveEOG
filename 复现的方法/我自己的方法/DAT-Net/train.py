"""
DAT-Net 训练脚本
Disentangling Attention Temporal-Network
双输出训练：EEG干净信号 + EOG伪影
"""
import os
import sys
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if current_cc > best_cc:
            best_cc = current_cc
            torch.save(model.state_dict(), 'DAT-Net_best.pth')
    ...
    # 保存最终模型
    torch.save(model.state_dict(), 'DAT-Net_final.pth')
    
    print('\n' + '='*70)
    print('训练完成!')
    print(f'最佳CC: {best_cc:.4f}')
    print(f'最佳模型已保存至: DAT-Net_best.pth')
    print(f'最终模型已保存至: DAT-Net_final.pth')
    print('='*70)
