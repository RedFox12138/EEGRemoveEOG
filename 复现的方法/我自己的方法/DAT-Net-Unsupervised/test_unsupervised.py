...
def main():
    ...
    model_path = 'DAT-Net-Unsupervised_best.pth'
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=device))
        print(f'加载模型: {model_path}')
    else:
        print('⚠️  找不到训练好的模型，尝试使用 final 版本...')
        model_path = 'DAT-Net-Unsupervised_final.pth'
        if os.path.exists(model_path):
            model.load_state_dict(torch.load(model_path, map_location=device))
            print(f'加载模型: {model_path}')
        else:
    ...
    with torch.no_grad():
        for noisy, clean, norm in loader:
            ...
            # 注意：训练时输入是归一化的 (noisy / norm)
            # 但训练时是在原始尺度下计算损失的 (在 unsupervised_artifact_v1.py 中会乘回 norm)
            # 所以测试时也需要在原始尺度下推理
            
            # 前向传播 - 使用原始尺度的数据
            noisy_t = noisy.float().unsqueeze(1).to(device)  # (B, 1, L)
            norm_t = norm.float().view(-1, 1, 1).to(device)  # (B, 1, 1)
            noisy_t = noisy_t * norm_t  # 恢复原始尺度
    ...
    save_path = os.path.join(out_dir, 'DAT-Net-Unsupervised_predictions.mat')
