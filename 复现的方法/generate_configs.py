...
MODEL_SAVE_PATH = os.path.join(CHECKPOINT_DIR, f'{method_name}_{{}}_best.pth'.format(DATASET_NAME))
FINAL_MODEL_PATH = os.path.join(CHECKPOINT_DIR, f'{method_name}_{{}}_final.pth'.format(DATASET_NAME))

# ========== 结果保存路径 ==========
RESULTS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'results')
os.makedirs(RESULTS_DIR, exist_ok=True)

PREDICTION_SAVE_PATH = os.path.join(RESULTS_DIR, f'{method_name}_{{}}_predictions.mat'.format(DATASET_NAME))


def print_config():
    \"\"\"打印当前配置信息\"\"\"
    print("=" * 80)
    print(f"{method_name} 数据配置信息")
    print("=" * 80)
    print(f"\\n[数据集配置]")
...
for folder, method_name in methods:
    config_path = os.path.join(base_path, folder, 'data_config.py')
    content = TEMPLATE.format(method_name=method_name)
