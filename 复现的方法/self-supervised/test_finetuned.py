"""
Self-Supervised EEG Denoising 微调模型测试脚本
加载微调后的模型进行测试
"""
import os
import sys
...
def main():
    print('='*70)
    print('Self-Supervised EEG Denoising 微调模型测试')
    print('='*70)
    ...
    # 加载微调后的模型权重
    model_path = 'Self-Supervised_finetuned_best.pth'
    if os.path.exists(model_path):
        model.load_state_dict(torch.load(model_path, map_location=device))
        print('加载微调模型:', model_path)
    else:
        print('⚠️ 找不到微调模型，尝试 final 版本...')
        model_path = 'Self-Supervised_finetuned_final.pth'
        if os.path.exists(model_path):
            model.load_state_dict(torch.load(model_path, map_location=device))
            print('加载微调模型:', model_path)
        else:
    ...
    print_metrics(metrics, prefix='测试集 (微调后)')

    # 保存结果
    out_dir = r'D:\Pycharm_Projects\EOG Remove\复现的方法\results'
    os.makedirs(out_dir, exist_ok=True)
    save_path = os.path.join(out_dir, 'Self-Supervised-Finetuned_predictions.mat')
