...
MODEL_SAVE_PATH = os.path.join(CHECKPOINT_DIR, f'SelfSupervised_{}_best.pth'.format(DATASET_NAME))
FINAL_MODEL_PATH = os.path.join(CHECKPOINT_DIR, f'SelfSupervised_{}_final.pth'.format(DATASET_NAME))

# ========== 结果保存路径 ==========
RESULTS_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'results')
os.makedirs(RESULTS_DIR, exist_ok=True)

PREDICTION_SAVE_PATH = os.path.join(RESULTS_DIR, f'SelfSupervised_{}_predictions.mat'.format(DATASET_NAME))


def print_config():
    """打印当前配置信息"""
    print("=" * 80)
    print(f"SelfSupervised 数据配置信息")
    print("=" * 80)
    print(f"\n[数据集配置]")
